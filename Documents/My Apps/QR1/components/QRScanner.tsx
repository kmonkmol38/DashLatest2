...
export const QRScanner: React.FC<QRScannerProps> = ({ onScanSuccess, onScanError }) => {
  ...
  const startScanning = async (facingMode: "environment" | "user" | { deviceId: string } = "environment") => {
    ...
    try {
      ...
      // If environment fails, try the default camera (no constraints)
      if (facingMode === "environment") {
        console.log("Retrying with default camera...");
        try {
          // Try to get any camera ID if constraints fail
          const devices = await Html5Qrcode.getCameras();
          if (devices && devices.length > 0) {
            await startScanning({ deviceId: devices[0].id });
          } else {
            throw new Error("No cameras found on device.");
          }
        } catch (retryErr: any) {
          setCameraError(retryErr.message || "Could not access any camera.");
          setUseCamera(false);
        }
      } else {
