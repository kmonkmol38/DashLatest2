...
const DriverOperatorView: React.FC<DriverOperatorViewProps> = ({ data, sharedFilters, setActiveView }) => {
  ...
  const summaryData = useMemo(() => {
    ...
    return { totalRevenue, totalExpense, totalProfit, recordCount };
  }, [filteredData]);
        ...
        <div className="flex items-stretch bg-secondary/20">
          ...
          <div className="bg-secondary flex flex-col justify-center p-2 ml-px flex-grow">
              ...
              <select
                ...
                className="bg-white text-black rounded-sm p-1 text-sm border-0 focus:ring-0 w-full"
              >
              {dynamicOptions.designations.map(designation => <option key={designation} value={designation}>{designation}</option>)}
              </select>
          </div>
        </div>
      </div>
